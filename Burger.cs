        ...
        public void Update(GameTime gameTime, KeyboardState keyboard)
        {
            ...
            if (health > 0)
            {
                ...
                // timer concept (for animations) introduced in Chapter 7
                 ...
                 //shoot if appropriate (using keyboard)
                if (keyboard.IsKeyDown(Keys.Space) && canShoot == true)
                {
                    Projectile fryProjectile = new Projectile(ProjectileType.FrenchFries, Game1.GetProjectileSprite(ProjectileType.FrenchFries),
                        drawRectangle.X + GameConstants.FRENCH_FRIES_PROJECTILE_OFFSET, drawRectangle.Y , GameConstants.FRENCH_FRIES_PROJECTILE_SPEED);

                    //play sound effect for shot
                    shootSound.Play();

                    //add created projectile
                    Game1.AddProjectile(fryProjectile);
                    canShoot = false;
                }

                //moves burger (using keyboard)
